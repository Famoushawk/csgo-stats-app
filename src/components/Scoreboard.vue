...
export default defineComponent({
  ...
  setup() {
    ...
    const matchStats = ref<{[key: string]: PlayerStatsData}>({});
    ...
    const transformPlayerStats = (roundSnapshot: RoundSnapshot): TransformedPlayerStats[] => {
      ...
      NAVI_PLAYERS.forEach(name => {
        const overallStats = matchStats.value[name] || {
          total_kills: 0,
          deaths: 0,
          headshots: 0,
        ...
        const roundStats = roundSnapshot.player_stats[name] || {
          total_kills: 0,
          deaths: 0,
          headshots: 0,
          headshot_percentage: 0,
        ...
        stats.push({
          name,
          kills: roundStats.total_kills,
          deaths: roundStats.deaths,
          headshots: roundStats.headshots,
          headshotPercentage: roundStats.total_kills > 0 ? (roundStats.headshots / roundStats.total_kills * 100) : 0,
      ...
      VITALITY_PLAYERS.forEach(name => {
        ...
        stats.push({
          ...
          deaths: roundStats.deaths,
          headshots: roundStats.headshots,
          headshotPercentage: overallStats.headshot_percentage,
          weaponBreakdown: Object.entries(roundStats.weapons || {})
            .reduce((prev, curr) => prev[1] > curr[1] ? prev : curr, ['', 0])
            .filter(v => v !== 0)
            .join(': ') || '-'
    ...
    onMounted(async () => {
      try {
        ...
        if (data.round_stats) {
          ...
          matchStats.value = data.player_stats; // Store the overall match stats
